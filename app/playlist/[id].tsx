...
import { getPlaylist as getMockPlaylist } from "../../constants/data";
...
export default function PlaylistScreen() {
  ...
  const { isAuthenticated, getPlaylist } = useSpotify();
    ...
    async function load() {
      ...
      try {
        if (isAuthenticated) {
          ...
          if (!cancelled) setPlaylist(getMockPlaylist(id!) ?? null);
        }
      } catch {
        if (!cancelled) setPlaylist(getMockPlaylist(id!) ?? null);
      } finally {
  ...
  }, [id, isAuthenticated, getPlaylist]);
